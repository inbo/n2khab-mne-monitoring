...
There are three options to enable "auto-login" / "passwordless" auth:
	- The **preferred option** is to use the password prompts built in to [[R/MNMDatabaseConnection|the custom R MNM database connection]].
	...
	- For [[R]] and [[python 1]]: better use a [[connection config file]]. Though storage of a password is possible, better don't store plain-text passwords here for users with write access. A good option is to store your config file in an encrypted container file (e.g. [[software/tomb]]) and mount it for interaction.
