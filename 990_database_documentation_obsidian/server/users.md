...
tags:
  - users
---
- *Disambiguation:* This is about server system users; for postgresql user roles, *cf.* [[authentication|authentication]]
...
- A special system user is the `postgres` user to manage the [[database/database]].
	- It is locked, but can be accessed by first switching to root user, and successively switching to the database user:
  ```sh
  su - root
  su - postgres
  ```
	- This is needed for [[database/creation|database generation]] and to handle [[authentication|user roles]].
