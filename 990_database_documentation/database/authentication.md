...
- The **preferred option** is to use the password prompts built in to [the custom R MNM database connection]([[MNMDatabaseConnection]]).
...
- For [[R]] and [[python]]: better use a [[connection config file]]. Though storage of a password is possible, better don't store plain-text passwords here for users with write access. A good option is to store your config file in an encrypted container file (e.g. [[tomb]]) and mount it for interaction.
...
```
## Troubleshooting
If a user *cannot logon* to the database (e.g. via [QField]([[qfield]])), there are the following levels of login prevention to be considered faulty:
- `pg_hba.conf` file (*cf.* [[postgresql]] ) restricts login, e.g. by preventing unknown IPs
	- this throws a quite instructive error message, mentioning `pg_hba`
	- -\> [ssh]([[ssh]]) to server, go `root`, and manually configure `/var/lib/postgres/data/pg_hba.conf`
	...
	- -\> login as `admin` db user and check table permissions:
		...
		- find all tables to which a user has access:
			...
			```sql
				...
				WHERE grantee = '<user>';
		```
- QGIS/QField login: data is in the database, but not visible in [QGIS]([[qgis]])
