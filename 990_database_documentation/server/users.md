...
A special system user is the `postgres` user to manage the [[database/database|database]].
- It is locked, but can be accessed by first switching to root user, and successively switching to the database user:
	```sh
	su - root
	su - postgres
	```
- This is needed for #database/creation and to handle [[database/authentication|user roles]].
