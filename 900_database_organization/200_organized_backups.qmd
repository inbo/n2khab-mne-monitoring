...
new_data <- dplyr::tbl(db_source, get_tableid(test_table)) %>% collect()
# TODO dropped NOT-NULL constraint in Visits

test_table <- "Protocols"
new_data <- read.csv(here::here("dumps", "Protocols_new.csv"))
# TODO restrict this to modified data, ignore empty rows
...
- `characteristic_columns` must be the intersect of possible columns in the case of slight db structure changes,
  or a link of renamend columns (`join_by`?)
