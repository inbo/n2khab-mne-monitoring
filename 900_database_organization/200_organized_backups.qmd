...
re_upload_tabledata(
  test_table = test_table,
  new_data =
    dplyr::tbl(db_source, DBI::Id(schema = "outbound", table = test_table)) %>%
      select(-targetpanel) %>% 
      collect(),
  characteristic_columns = c("scheme", "module_combo_code", "panel_set", "stratum", "grts_address"),
  rename_characteristics = c("type" = "stratum", "type_adjusted" = "stratum_to_expect")
)
...
re_upload_tabledata(
  ...
  rename_characteristics = c("activity_rank" = "activity_sequence", "type_assessed" = "stratum_corrected")
)
