...
if restore_dev:
    # database: mnmsurfdb_dev
    db_connection = DTB.ConnectDatabase(
        base_folder/"mnm_database_connection.conf",
        connection_config = "mnmsurfdb-dev",
        )
    db = DTB.Database( \
        structure_folder = base_folder/"mnmsurfdb_dev_structure", \
...
if restore_staging:
    ...
    db_connection = DTB.ConnectDatabase(
        base_folder/"mnm_database_connection.conf",
        connection_config = "mnmsurfdb-staging",
        )
    db = DTB.Database( \
        structure_folder = base_folder/"mnmsurfdb_db_structure", \
        definition_csv = "TABLES.csv", \
        lazy_creation = False, \
        db_connection = db_connection, \
        tabula_rasa = True
...
if restore_testing:
    ...
    db_connection = DTB.ConnectDatabase(
        base_folder/"mnm_database_connection.conf",
        connection_config = "mnmsurfdb-testing",
        )
    db = DTB.Database( \
        structure_folder = base_folder/"mnmsurfdb_dev_structure", \
        definition_csv = "TABLES.csv", \
...
if False:
    ...
    # database: mnmsurfdb PRODUCTION
    db_connection = DTB.ConnectDatabase(
        base_folder/"mnm_database_connection.conf",
        connection_config = "mnmsurfdb",
    )
    db = DTB.Database( \
        structure_folder = base_folder/"mnmsurfdb_db_structure", \
        definition_csv = "TABLES.csv", \
