...
Details are documented in the [[https://github.com/inbo/n2khab-mne-monitoring/blob/main/990_database_documentation/database/backups.md][MNE database documentation]].
...
There are python scripts for this purpose.

#+begin_src python :eval no :tangle 931_create_empty_testing.py
# DO NOT MODIFY
...
but you would like to partially copy or append from a /source/ to /target/.
...
#+begin_src R :eval no :tangle 932_production_to_testing_example.R
# DO NOT MODIFY
# this file is "tangled" automatically from `930_copy_database.org`.
...
20260616: join issue duplicate characteristics on =MHQPolygons=
#+begin_src sql

SELECT DISTINCT location_id, grts_address, count(*) AS N
FROM "outbound"."MHQPolygons"
GROUP BY location_id, grts_address
ORDER BY N DESC
LIMIT 10
...
#+begin_src R :eval no :tangle 933_populate_testing_db.R
# DO NOT MODIFY
# this file is "tangled" automatically from `930_copy_database.org`.
...
*** fix crs mismatch when copying polygon cells
CLOSED: <2026-06-01 Mon>

The issue here was a combination of cascaded deletion and therefore an unexpected empty table.

The loop above did not work (it cascade-deleted LocationCells each time I tested), whereas the code below would execute flawlessly:

#+begin_src R :eval no :tangle no
# to be run after manual `populate_testing_db.R`

table_idx = 8
table_label <- "LocationCells"
source_data <- source_db$query_table(table_label)
target_data <- target_db$query_table(table_label)
...
** fix =location_id IS NULL=

#+begin_src python :eval no :tangle no :echo no

# TODO due to ON DELETE SET NULL from "Locations", location_id's temporarily become NULL.
#      Updating would be cumbersome.
constraints_mod_OBSOLETE <- function(do = c("DROP", "SET")) {
