...
# TODO password query does not work in a quarto render. -> use manually
# keyring::key_set("DBPassword", "loceval_dbadmin")
...
if (reload || !file.exists(poc_rdata_path)){
  drive_download(as_id("1a42qESF5L8tfnEseHXbTn9hYR1phqS-S"), path = poc_rdata_path)
}
load(poc_rdata_path)
```
...
connect_database_configfile <- function(config_filepath, database, config_choice = 1, password = NULL) {

  # read connection info from a config file
  config <- configr::read.config(file = config_filepath)[[config_choice]] # selects config block
...
if (working_dbname == "loceval") {
  ...
  db_connection <- connect_database_configfile(
    ...
    config_choice = 2,
    password = keyring::key_get("DBPassword", "loceval_dbadmin")
  )
...
- `rank` is renamed to `activity_sequence`.
