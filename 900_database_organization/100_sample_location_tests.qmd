...
activity_lookup <- upload_and_lookup(
  db_connection,
  DBI::Id(schema = "metadata", table = "Activities"),
...
- [X] metadata table for habitat/stratum:
  - `n2khab_types_expanded_properties` -> `N2kHabTypes`
  - `CHECK ((stratum_to_expect IS NULL) OR (stratum_to_expect IN (SELECT DISTINCT type FROM "metadata"."N2kHabType"))`
- [ ] find meaningful summary table of "activities"
