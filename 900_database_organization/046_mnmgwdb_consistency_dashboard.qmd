---
title: "MNMGWDB Consistency Dashboard"
date: last-modified
format:
  html:
    toc: true
...
check_foreignkey_connection(
  table_label = "Visits", fk = "location_id",
  characteristic_columns = c("grts_address"),
  ref_table = "Locations", pk = "location_id"
)
...
```{r random-placement-points}
#| eval: true
#| echo: false

check_foreignkey_connection(
  table_label = "InstallationPoints", fk = "location_id",
  characteristic_columns = c("grts_address"),
  ref_table = "Locations", pk = "location_id"
)

check_foreignkey_connection(
  table_label = "InstallationPoints", fk = "sampleunit_id",
...
("archive" tag set along the line of `SampleUnits` -> `FieldCalendars` -> `Visits`)

```{r consistent-archive}
#| eval: false
#| echo: false

archives <- mnmgwdb$query_table("SampleUnits") %>%
  left_join(
