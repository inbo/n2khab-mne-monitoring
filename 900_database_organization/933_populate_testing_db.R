...
dateconvert_fcn <- \(x) if (is.na(x)) return(NA) else convert_timestamp_to_ms_character(lubridate::as_datetime(x))

handle_datetime_column <- function(visits) {
  if (nrow(visits) == 0){
    visits_modified <- visits %>% mutate(datetime_visit = as.character(datetime_visit))
  } else {
  visits_modified <- visits %>%
    dplyr::mutate(datetime_visit = unlist(purrr::map(datetime_visit, dateconvert_fcn)))
  }
...
table_modification <- c(
  ...
  "TargetPoints" = \(tp) tp %>% mutate(log_creation = unlist(purrr::map(log_creation, dateconvert_fcn)))
