...
- <20260309> check that the order of sideloading is sufficient:
  - first: get `Locations` GRTS list from {status quo database + rep news}
  - much later: sideload
  - sideloaded, ex-REP locations should be uploaded, too
  - but then, how would their `archive_version_id` status be?
...
sample_units_old %>% names()
sample_units_old %>% count(is_replacement)
