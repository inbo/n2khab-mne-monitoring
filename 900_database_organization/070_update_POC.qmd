...
2. Mirrors: try everything on `testing` and then `staging`, first.
  - The `testing` mirror (data sync with `033_populate_testing_db.R`) is open for play and can taken even rough rides, though note that user permissions differ on there.
  - On `staging`, everything is exactly identical to the production server, because it gets synced with the commands below.
...
```
  ...
  - if necessary, re-instantiate the mirror prior to data upload (and then run `110_*` and `210_*`, if necessary)
  - Finally, use the `pg_dump | psql` command chain above above.
...
base_path = "/data/git/n2khab-mne-monitoring_support"
base_path = "/work/inbo/git/n2khab-mne-monitoring_support"
load_poc_code_snippets(base_path)
```
...
update_cascade_loceval <- parametrize_cascaded_update(
  loceval
)
...
Replacements are tabula-rasa'd below.
...
## sample unit polygons


For the sample locations, we also need to see polygons (replacement procedure, habitat mapping).

Those come from the habitat map.

```{r load-location-polygons}

# only get the polygons for cells in the sample
...
# load and select from habitat map
...
bind polygons to the sample units

```{r bind-polygons-to-sample-units}
...
this is stored on the postgis server.
