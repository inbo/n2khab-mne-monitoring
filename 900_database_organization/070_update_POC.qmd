...
system("source .dbinit/bin/activate && python 092_push_loceval_to_mnmgwdb.py")
```
...
```{sql}
-- SELECT * FROM "inbound"."WellInstallationActivities" WHERE grts_address = 219694 AND activity_group_id = 4 AND fieldwork_id = 5449;
-- SELECT * FROM "inbound"."ChemicalSamplingActivities" WHERE grts_address = 219694 AND activity_group_id = 13 AND fieldwork_id = 5675;
```
...
new_locinfos <- new_locinfos %>%
  anti_join(
    mnmgwdb$get_table("LocationInfos"),
    by = join_by(grts_address)
  ) %>%
  left_join(
    locations_lookup,
