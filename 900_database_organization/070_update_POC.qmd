...
if (FALSE) {
  ...
  version_notes <- "dolle dromedaris: aanpassing van steekproefgroottes op typeniveau (verkleiningen resp vergrotingen)"
...
# data_future %>% select(!!!characteristic_columns) %>% saveRDS("./dumps/datelink_older.rds")
# data_future %>% select(!!!characteristic_columns) %>% write.csv2("./dumps/datelink_older.csv")
distribution <- categorize_data_update(
  mnmdb = mnmgwdb,
  table_label = table_label,
  data_future = data_future,
