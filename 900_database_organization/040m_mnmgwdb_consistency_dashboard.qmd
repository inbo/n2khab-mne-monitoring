...
-- FROM "inbound"."WellInstallationActivities" AS SACT
-- FROM "inbound"."ChemicalSamplingActivities" AS SACT

SELECT *
FROM "inbound"."ChemicalSamplingActivities" AS SACT
LEFT JOIN "outbound"."SampleLocations" AS SLOC
  ON (SLOC.grts_address = SACT.grts_address)
  AND (SLOC.strata = SACT.stratum)
WHERE SACT.grts_address = 437685;

NOT(SLOC.samplelocation_id = SACT.samplelocation_id)
;

SELECT * FROM "outbound"."SampleLocations" WHERE grts_address = 437685;
SELECT * FROM "outbound"."ChemicalSamplingActivities" WHERE grts_address = 437685;

vervanging  437685 -> 8826293
```
...
check_foreignkey_connection(
  table_label = "SpatialPositioningActivities", fk = "samplelocation_id",
  characteristic_columns = c("grts_address", "stratum"),
  ref_table = "SampleLocations", pk = "samplelocation_id",
  rename_ref = c("strata" = "stratum")
  )
...
check_foreignkey_connection(
  table_label = "SpatialPositioningActivities", fk = "fieldworkcalendar_id",
  characteristic_columns = c("grts_address", "stratum", "activity_group_id", "date_start"),
  ref_table = "FieldworkCalendar" 
  )
...
check_foreignkey_connection(
  table_label = "SpatialPositioningActivities", fk = "visit_id",
  characteristic_columns = c("grts_address", "stratum", "activity_group_id", "date_start"),
  ref_table = "Visits" 
  )
