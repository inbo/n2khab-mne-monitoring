...
In all brevity, I collect steps here to get a new RVP to the database.

# Notes and TODOs

- <20260309> check that the order of sideloading is sufficient:
  - first: get `Locations` GRTS list from {status quo database + rvp news}
  - much later: sideload
  - sideloaded, ex-RVP locations should be uploaded, too
...
if (isFALSE(grepl("-staging", mnmgwdb_mirror))) {
  mnmgwdb$dump_all(
    here::here("dumps", glue::glue("rvp_pre_update_{mnmgwdb$database}_{now}.sql")),
    exclude_schema = c("tiger", "public")
  )
}
...
if (FALSE) {
  ...
  version_notes <- "correct filter for post 2026 locations"
...
field_activities_db <- mnmgwdb$query_table("GroupedActivities") %>%
  filter(is_gw_activity, is_field_activity) %>%
  distinct(activity_group, activity)
...
system(glue::glue("Rscript 111a_push_loceval_to_mnmgwdb.R {mirror}"))
...
The original sample location gets archived during the next RVP update script - here.

At the time of writing, this seems to be a logical solution. 
However, on a real RVP update, the locations to archive because of local replacement ("downstream" archives) get mixed with actual RVP changes ("upstream").
:::
...
# stop("CONSIDER: these are not special activity characteristic columns!")
...
startdate_updates_happened <- associate_and_shift_start_dates(
  ...
  data_future = data_nouveau,
  characteristic_columns = characteristic_columns,
  other_table_labels = c("Visits", special_activity_tables)
) 
...
if (isFALSE(grepl("-staging", mnmgwdb_mirror))) {
  mnmgwdb$dump_all(
    here::here("dumps", glue::glue("rvp_post_update_{mnmgwdb$database}_{now}.sql")),
    exclude_schema = c("tiger", "public")
  )
}
...
system("quarto render 046_mnmgwdb_consistency_dashboard.qmd --to html")
```
...
message("")
message("________________________________________________________________")
message(" >>>>> Finished RVP data update. ")
message("________________________________________________________________")
```
