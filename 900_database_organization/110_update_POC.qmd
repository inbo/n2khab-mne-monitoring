...
```{r reference-locations}
online_locations <- dplyr::tbl(
    db_connection,
    DBI::Id(schema = "metadata", table = "Locations"),
  ) %>% collect() %>%
  select(location_id, grts_address)


online_samplelocations <- dplyr::tbl(
    db_connection,
    DBI::Id(schema = get_schema("SampleLocations"), table = "SampleLocations"),
  ) %>% collect() 


# TODO SampleLocations can be deleted; will be re-uploaded below.
if (FALSE){
  execute_sql(
    db_connection,
    glue::glue("DELETE FROM {get_namestring('SampleLocations')};"),
    verbose = FALSE
  )
}
```
...
new_locations %>%
  filter(grts_address == 230930)
```


# TODO stuck here:

:::{.callout-warning title="quick diagnosis:"}
This seems to be a dbplyr/sf issue.
Try wrapping the upload into hand-made INSERT commands.
:::
...
locations_lookup <- upload_and_lookup(
  ...
  DBI::Id(schema = "metadata", table = "Locations"),
  new_locations,
  ref_cols = c("grts_address"), # uniqueness-issue
