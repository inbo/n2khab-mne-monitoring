...
- 20260703 REP update 0.17.0
- 20260811 quarterly prio update 
- 20260819 REP 0.18.0
- 20260925 intrinsic matching occasions (Visits 1 : n FieldCalendars)
...
if (isFALSE(grepl("-staging", mnmsurfdb_mirror))
    && isFALSE(grepl("-testing", mnmsurfdb_mirror))
    && isFALSE(grepl("-dev", mnmsurfdb_mirror))
    ) {
  mnmsurfdb$dump_all(
...
if (FALSE) {
  ...
  # version_tag <- "mnmsurfdb_initialization" # "REP + snippets update 0.16" # only used for testing
  # version_tag <- "snip:0.17_rep:0.17" # data_iteration = 1
  # version_notes <- "activating SURF_03.4_LENTIC"
  # version_notes <- "shifting aquatics from GRTS cell to point on polygon surface"
  # version_notes <- "quarterly prio update"
  # version_tag <- "snip:0.18.0_rep:0.18.0" # version_notes <- "REP 0.18.0 refinements on lentic sample units"

  version_tag <- "snip:0.18.0_rep:0.18.0" # data_iteration = 1
  version_notes <- "realize matching occasions ('intrinsic')"
...
mnmsurfdb_freeze <-
  ...
  mutate(matching_occasion = NA, date_suggested = date_start)
...
data_changed <- distribution$changed %>% 
  arrange(!!!rlang::syms(sampleunit_characols))

data_previous <- mnmsurfdb$query_table(
    table_label,
    ONLY = isFALSE(mnmsurfdb$is_spatial(table_label))
  ) %>%
  semi_join(
    data_changed,
    join_by(!!!rlang::syms(sampleunit_characols))
  ) %>%
  select(
...
```{r sampleunit-polygons}

sampleunit_polygons <- aquatic_location_polygons %>%
  left_join(
    sampleunits_lookup,
    by = join_by(grts_address, stratum)
  ) %>%
...
fieldcalendar_updated <- fieldwork_shortterm_prioritization_by_stratum %>%
  ...
  select(tidyselect::any_of(fieldcalendar_columns))
...
fieldcalendar_new <- fieldcalendar_new %>%
  # select(-tidyselect::any_of(date_suggested)) %>% 
  left_join(
    matching_occasions,
...
# data_ancient <- mnmsurfdb$query_columns(
#   "FieldCalendars",
#   colnames(data_nouveau)
# )
# data_nouveau %>% glimpse()
# data_ancient %>% glimpse()
# 
# waldo::compare(
#   data_nouveau %>% arrange(grts_address, stratum, date_start, activity_group_id),
#   data_ancient %>% arrange(grts_address, stratum, date_start, activity_group_id
# )
```
...
mnmsurfdb$query_table("FieldCalendars") %>% 
...
fieldcalendars_aggregated <- mnmsurfdb$query_columns(
    "FieldCalendars"    ,
    c(fieldcalendar_characols,
      "matching_occasion",
      "fieldcalendar_id",
      "sampleunit_id"
      )
  ) %>%
  arrange(stratum, activity_group_id, grts_address, fieldcalendar_id) %>% 
  summarize(
    fieldcalendar_ids = list(fieldcalendar_id),
    stratums = list(stratum),
    sampleunit_ids = list(sampleunit_id),
    .by = c(grts_address, activity_group_id, date_start, matching_occasion)
  )

# fieldcalendars_aggregated %>% glimpse()

potential_visits <- fieldcalendars_aggregated %>%
  select(
    tidyselect::any_of(c(
      visits_characols,
      "stratums", "sampleunit_ids", "fieldcalendar_ids"
    ))
  ) %>%
  left_join(
    locations_lookup,
...
selection_of_activities <- list(
  "LenticVisits" = activity_groupid_lookup %>%
    filter(grepl("^SURFLENT", activity_group)) %>%
  ...
  "LoticVisits" = activity_groupid_lookup %>%
    filter(grepl("^SURFLOT", activity_group)) %>%
...
# quick fix: convert array cols to string manually
table_info <- mnmsurfdb$load_table_info("Visits")
table_info %>% pull(datatype)
array_columns <- table_info %>%
  dplyr::filter(grepl("array|[[]]", tolower(datatype))) %>%
  dplyr::pull(column)


if (length(array_columns) > 0 && nrow(remaining_new_visits) > 0) {
  wrap_curls <- \(arr_str) paste0(c("{", arr_str, "}"), collapse = "")
  listpaste <- \(arr) paste0(unlist(arr), collapse = ",")
  upload_prep <- \(x) wrap_curls(listpaste(x))

  # upload_prep(remaining_new_visits[1, "stratums"])
...
mnmsurfdb$query_table("Visits") %>% 
  count(
    is.na(location_id),
    is.na(sampleunit_ids),
    is.na(fieldcalendar_ids)
  ) %>%
  knitr::kable()
...
srctab <- mnmsurfdb$get_namestring("FieldCalendars")
link_key_column <- "archive_version_id"
lookup_criteria <- c("SRCTAB.visit_id = TRGTAB.visit_id ")
...
mnmsurfdb$query_table("FieldCalendars") %>%
  anti_join(
    mnmsurfdb$query_table("VisitsUnnested"),
...
out <- processx::run(
  "quarto",
  c("render", "047_mnmsurfdb_consistency_dashboard.qmd", "--to", "html"),
  echo = TRUE,
  spinner = TRUE
)
