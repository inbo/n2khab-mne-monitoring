...
+ [ ] 20250704 loceval post mortem
  + [ ] review the "POC Update" procedure (++ExtraVisits)
  + [ ] new locations to assess??
    + recover old RData and compare
  + [ ] "verouderd" field is rubbish -> correct OrthophotoAssessment view
  + [ ] loc_id mismatch: "schietveld" in Hemiksem?
  + [ ] combined UNIQUE constraints for all characteristic columns
  + [ ] reset sequences on tabula rasa / DELETE+INSERT updates

+ [ ] backup procedures
  + [ ] daily backups to S3
+ [ ] list WCS/WFS/WMS sources
+ [ ] random points in target area for well placement
+ [ ] re-upload bug -> was fk
