...
+ [ ] input meeding @WT @KW @FV @FM <2025-06-24 Tue>
  + [ ] decoupling LocCal - Visits!!
  + [ ] =ortophoto_2025_cells= and =ortophoto_2025_cellcenters=
  + [ ] orthophotos:
    + via "datavindplaats" -> wcs/wfs
    + winter && summer
  + [ ] use =grts_address_final=
  + [ ] =ortophoto_2025_type_grts=
    + assessment
    + sp_poststratum
  ...
  + [ ] habmap: use 2023 fixed version; later: preliminary update by @FV
  + [ ] protocol-like criteria by @WT
  + [ ] qgis design
    + [ ] types -> qgis map label
    + [ ] "activity" visible in form
    + [ ] test attribute table sorting
    + [ ] add habitatmap (raw; wfs?) as bachground layer
...
+ [-] suggestions @FV <2025-06-18 Wed>
  ...
  + [ ] remove =targetpanel=; replace by =fieldwork_2025_priorities= + =stratum_scheme_ps_targetpanel= (for Tom)
  ...
  + [ ] corner points or polygons for sample units (also depends on type)
