...
+ [X] UPDATE... FROM
  e.g. =UPDATE "outbound"."Replacements" AS REP SET grts_address = UNIT.grts_address FROM "outbound"."SampleUnits" AS UNIT WHERE REP.sampleunit_id = UNIT.sampleunit_id;=
+ ... in combination with TEMP TABLES for cascaded update  
  use =temporary= in https://dbi.r-dbi.org/reference/dbWriteTable.html
+ [X] ... and for stitching


+ 20250916 Falk, is het eventueel mogelijk om bij een vervangcel in de planning ook het orginele GRTS-adres op te geven? Momenteel is het voor mij moeilijk om uit te zoeken of een vervangcel gekoppeld is aan een cel van de nieuwe planningslaag (voorlopig geopackage van Floris)
  + [X] via "archive"."ReplacementData"
  + [ ] better store another "grts_original"

+ [X] Vraag Tom 20250918: zou je aan de FieldworkPlanning in outbound nog een attribuut kunnen toevoegen voor het inplannen van de waterstalen? : count_days_ws:  day(to_date(now())-to_date( "date_vis_1"))

+ [X] 219694 and 4447925: no activities? (Tom 20250919)

20250923:
+ [X] re-run maintenance tasks (carefully)
<<<<<<< HEAD
+ [X] yet another pocupdate =mnmgwdb=
+ [ ] also update =loceval=
+ [ ] map layer with "placed wells"

+ [ ] data consistency dashboard -> loceval

=======
+ [ ] yet another pocupdate =mnmgwdb=
+ [ ] also update =loceval=
+ [ ] map layer with "placed wells"

>>>>>>> 5c6f9a5 (dbinit: (wip) another POC update/testing)
prio:
+ [ ] postgresql update 20250925
  + [X] announced
  + [ ] local testing
  + [ ] ... and action!
+ [ ] !!! re-think =SampleLocations= dependency!
  + was made implicit: no direct fk linkage of tables
  + this cuts ties upon update
  + =samplelocation_id= must be manually re-linked everywhere
  + shortcut alternative: retry setting foreign key everywhere
  ==> better explicit `fk` with cascade-NULL
+ [ ] see chat and mails by placement team for manual adjustments
+ [ ] also update latest POC and RData


from POC Update:
+ [ ] also update =loceval=
+ [ ] more consistent =UNIQUE()= rules on all characteristic columns
+ [X] a "consistency dashboard" with checks for NA's and correct links
+ [ ] script for common reset =fieldwork_id=
...
from last fieldwork meeting:
+ [ ] qfield visibility of fieldwork todo's
+ [ ] extra activities: ad-hoc tasks, replacement/repair, possibly/partly related to WIA
+ [ ] consistent color codes (use Tom's style files)
+ [ ] regions with special protection amphibia
+ [ ] data types logger read-outs


+ [ ] manual entries from fieldwork
+ [ ] =SELECT * FROM "inbound"."WellInstallationActivities" WHERE visit_id IS NULL;=
+ [ ] =SELECT * FROM "inbound"."ChemicalSamplingActivities" WHERE visit_id IS NULL;=
+ [ ] 871030 restore 4010
+ [ ] 20250916 why can FW's not write "veldwerk voor iedereen"?
+ [ ] 6092977 did they visit already?

+ [X] continue =20250903_missing_871030.sql=
+ [ ] delete =loceval::"outbound"."FieldActivityCalendar".landowner=
+ [X] there is a duplicate location in loceval: 871030, of course.
+ [ ] there is a duplicate location in loceval: 219694, of course.
+ [X] free fieldnote counter issue in =090_*=: =SELECT setval('seq_fieldnote_id', (SELECT MAX(fieldnote_id) FROM "inbound"."FreeFieldNotes"));=
...
>>>>>>> 4b8e3f7 (dbinit: uploading POC data to database (!!))
