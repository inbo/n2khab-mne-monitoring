...
+ [ ] gmaps attachment link (Ward/Tom)

+ [ ] qgis/qfield authentification for project sharing, there must be something online
+ [ ] soilmap_simple as gpkg layer
+ [ ] cell mapping
  + [ ] ?? why are some sample raster cells not visible?
+ [ ] new fields
  + [ ] divers: free field for exotic divers / re-use
  + [ ] two fotos soil profiles
  + [ ] checkbox for requiring "soil profile revision"
  + [ ] LIMS: project code + recipient code
...
+ [ ] divers: batch, serial; ++ view with previously placed

* POC
+ [ ] =mnmgwdb=:
  + [ ] use of =stratum=
  + [ ] central unit is NOT sample unit, but location ~> =SampleLocations= (because we will place exactly one obswell, no matter how many schemes/strata)
...
+ [ ] double-check =is_gw_activity=
+ [ ] no option to add activities / rows in FieldActivityPlanner?
+ [ ] what to do with the CellMaps? (e.g. transfer mnmgwdb?)
