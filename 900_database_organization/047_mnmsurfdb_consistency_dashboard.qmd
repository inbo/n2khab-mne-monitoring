---
title: "MNMSURFDB Consistency Dashboard"
date: last-modified
format:
  html:
    toc: true
...
# connect mnmsurfdb
mnmsurfdb_database <- glue::glue("mnmsurfdb{mirror}")

mnmsurfdb <- connect_mnm_database(
  config_filepath,
  database_mirror = mnmsurfdb_database,
  user = "monkey",
  password = NA
) 
...
count_table_content <- function(table_list) {
  lapply(
      mnmsurfdb$query_tables_data(table_list),
      FUN = nrow
    ) %>%
    as_tibble() %>%
    t() %>% 
...
check_foreignkey_connection <- function(
  ...
  table_data <- mnmsurfdb$query_table(table_label)
  ...
  ## mismatches
  ref_data <- mnmsurfdb$query_table(ref_table)
  if (!is.na(rename_ref)) {
  ...
  if (nrow(mismatch) > 0) {
    ...
    print(table_anti %>%
      select(!!!rlang::syms(c(
        mnmsurfdb$get_primary_key(table_label),
        characteristic_columns,
        fk
      ))),
      n = Inf
...
`SampleUnits` are currently not hard-referenced, i.e. I de-activated the foreign key constraint in favor of quick-and-dirty processing at some point.

:::


```{r field-calendar-sloc}
#| eval: true
#| echo: false

check_foreignkey_connection(
  table_label = "FieldCalendar", fk = "sampleunit_id",
...
count_archived_content(
    table_list = c("SampleUnits", "FieldCalendar", "Visits")
  ) %>%
  knitr::kable()
...
("archive" tag set along the line of `SampleLocation` -> `FieldCalendar` -> `Visits`)

```{r consistent-archive}
#| eval: true
#| echo: false

archives <- mnmsurfdb$query_table("SampleUnits") %>%
  left_join(
