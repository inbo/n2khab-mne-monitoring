#+title: Memorandum (/post mortem/): POC Update 0.14.0
#+date: 20251016-20251024


* DISCLAIMER

This document is a subjective, /ad hoc/ summary by Falk Mielke, and has not been reviewed by all stakeholders.
There are certainly points to add, and feedback is welcome via PR or github comment to [[https://github.com/inbo/n2khab-mne-monitoring/blob/dbinit_pocupdate014/900_database_organization/documentation/202510_pocupdate014_postmortem.org][this document]].
Some follow-up TODO's are not finished up at the time of writing, but will be as soon as possible.
...
+ [X] LOGICAL 2: (to be followed-up) "Achterhaven Zeebrugge" manual addition of out-of-sample locations in temporarily accessible area (loceval)
...
|                         |               | manual re-activation of [[Addresses Achterhaven][Addresses Achterhaven]]                                    |
|                         |               | - first in loceval                                                               |
...
|                         |               | installed.                                                                       |
|-------------------------+---------------+----------------------------------------------------------------------------------|
| =23/10/2025=              | @FV, @FM      | (some input from regular meeting with Jo / databeheer)                           |
|                         |               | follow-up and manual re-activation locations in Achterhaven Zeebrugge            |
|-------------------------+---------------+----------------------------------------------------------------------------------|
| =23/10/2025=              | @FM           | bugfix: missing some tables in =095_re_link_foreign_keys_optional.R= script        |
|-------------------------+---------------+----------------------------------------------------------------------------------|
| =24/10/2025=              | @FM           | ensure locations are planned/scheduled for autumn break                          |
|                         |               | manual re-activation of four confirmed locations in "De Liereman"                |
|                         |               | wrap-up this memorandum; to be followed up                                       |
|-------------------------+---------------+----------------------------------------------------------------------------------|
...
* Mitigations and TODO's
/(from discussion with Floris, 2025/10/21)/

+ *LOGICAL 1* persistence/linkage of =start_date= per =activity_group= as an event characteristic (rationale: "first/second installation/sampling/..."; given limited time interval)
  + temporal coupling: link+shift previous to new calendar entries
    + via "series of events" (grts+stratum+FAG+"series/rank")
    + both intra- and inter-activitygroup considerations
  + print-out and warning of changing events, to follow up by planners
  + if non-match:
    + mismatches, from theoretical standpoint, "should not have been planned"
    + yet there is always a /veto/ by Karen/Tom to recover; risk: different location or time, non-sampled location
...
+ FW (Fieldwork) Precedence: whether or not to keep out-of-sample locations
  + bias by out-of-sample or extra locations: depends
    ...
    + therefore generally: new plans must overwrite old plans
  + date shifts: likely
    + must be covered by database
    + analysis 
+ prohibit involuntary/premature changes on =production=
  + technically notify (= warn, pop-up, captcha-level of annoyance) upon any pushes to =production= mirror
  + check database/table/entry locks (suggestion by Jo)
+ anticipation
  + FM involved in data revisions as soon as possible
  + potentially use =_future= mirror
  + geopackages parallel to RData continued as checksums and confirmation 
  + avoid "special ops + update + vacation" situation
...
Note: those are likely to be only date shifts


** Addresses Achterhaven
See also detail planning [[https://docs.google.com/spreadsheets/d/1ZqEvJCS93BNZ_nftED7N4mO4J3sTyCqcwFQjun0nBaw][in this document]].
