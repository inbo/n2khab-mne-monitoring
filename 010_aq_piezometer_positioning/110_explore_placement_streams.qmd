...
```{r resample-linestream}

resample_linestream <- function(linestream, resample_m = 1) {
...
get_linestream_curve <- function(vhag, segment_rank = 0, resample_m = NA) {
  ...
  if (!is.na(resample_m)) {
    target_stream_points <- bind_rows(lapply(
      1:nrow(target_stream_points),
...
for (uid in seq(781, 852, 5)) {
  ...
  # get stream info
  point_info <- get_linestream_curve_properties(unit_id, smooth_range = 1, resample_m = 10)
...
for (i in seq(771, 852, 5)) {
  ...
  # flow and curvature
  scale <- 64
  point_info <- get_linestream_curve_properties(unit_id, resample_m = 10)
  segment_point <- point_info[c("x", "y")]
