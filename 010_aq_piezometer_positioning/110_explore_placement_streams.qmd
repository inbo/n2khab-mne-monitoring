...
# latest (20250506):
#   sample_filepath <- file.path("./data", "objects_for_aq_piezometers_panfl_pan5.RData")
...
if (force_reload_sample || !file.exists(sample_filepath)) {
...
stream_testpoints <- n2khab::read_watercourse_100mseg(element = "points") %>% 
  filter(vhag_code == 9574)
stream_testlines <- n2khab::read_watercourse_100mseg(element = "lines") %>% 
  filter(vhag_code == 9574)

mapview::mapview(stream_testpoints, col.regions = "brown") +
    mapview::mapview(stream_testlines, col.regions = "steelblue")
...
test_curve <- get_linestream_curve(
    ...
    streamline_datasource = stream_lines,
    resample_m = 1
  ) %>%
  filter(rank > 20)
...
# head(get_linestream_curve(8501, reference_rank = 224807, streamline_datasource = stream_lines))
# head(get_linestream_curve(8576, reference_rank = 98821, streamline_datasource = stream_lines))
# head(get_linestream_curve(vhag_code = 9574, reference_rank = 218821, streamline_datasource = stream_lines))
# 
```
...
get_linestream_target_area <- function(unit_id, offset_m = 2, side_m = 24) {
  ...
  # get stream info
  point_info <- get_linestream_curve_properties(
    unit_id, 
    smooth_range = 5, 
    resample_m = 10
    )
