...
# - Watervlakken 2024: https://doi.org/10.21436/inbor.114075267
# - `n2khab` function to read them: https://inbo.github.io/n2khab/reference/read_watersurfaces_hab.html
...
sample_filepath <- file.path("./data", "objects_panflpan5.RData")

if (force_reload_sample || !file.exists(sample_filepath)) {
  download_rep_rdata_file(sample_filepath, overwrite = TRUE)
}
...
(adjusted to read full REP `.RData`, [see here](https://github.com/inbo/n2khab-mne-design/blob/7a08ab9d36669d3a5b64d12b8bc35ff7c7a6a5d1/100_design_common/010_prototyping_sampling_and_revisit_design/helper_scripts/support_dcp_and_fieldwork.R))
...
stratum_units_grts_aquatic_gw_spsamples_spares <-
  ...
  filter(
    ...
    # # terrestrial spring units must also be excluded: # -> not for this notebook
...
In database jargon: we work on *Locations*, not on *SampleUnits*.
:::
...
:::{.callout-note}
Change 09/2026:
We better use the `sf::st_point_on_surface()` function
to be consistent with the `Location` point in REP and `mnmsurfdb`.
Implications are inferred experimentally.
:::
...
DONE (1) revisited this issue: had to do with the wrong data source.
DONE (2) using `sf::st_point_on_surface()` to make sure points are within the target water body; consistent with REP approach.
...
```{r radius}
nonnan <- function(vec) vec[!is.na(vec)]
nanmean <- function(vec) mean(vec, na.rm = TRUE) # mean(vec[!is.na(vec)])
...
# outsourced to "calculate_watersurface_flow_direction.R"
```
...
for (location_code in c(
  ...
  test_polygon <- watersurfaces_polygons %>%
    filter(unit_id == location_code)
  flow_vector <- calculate_watersurface_flow_direction(
    test_polygon,
...
upstream <- t(as.matrix(-calculate_watersurface_flow_direction(water, 64, 8, 8)))
...
flow_dir <- calculate_watersurface_flow_direction(
  test_polygon,
  flow_range = 128,
  flow_cellsize = 32,
  close_island_size = 8
...
- [X] `rank` in streams seems to be faulty? (see example zig-zag in higher ranks) -> *no. was just ggplot.*
