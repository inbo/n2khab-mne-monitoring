...
#'   calculate_flow_direction(location, buffer_width = 1024, flow_cellsize = 32)
#' }
#'
...
upstream_vec <- wedge_range * upstream / norm(upstream)
norm(upstream_vec)
...
determine_target_area <- function(
  ...
  center_location <- st_centroid(water_polygon) %>%
    suppressWarnings()
  ...
  pt0 <- t(as.matrix(c(0, 0)))
  upstream <- t(as.matrix(-calculate_flow_direction(center_location, ...)))
  upstream_vec <- wedge_range * upstream / norm(upstream)
  # norm(upstream_vec)
  ...
  triangle <- st_combine(triangle) %>% st_cast("POLYGON")
...
upstream_vec <- 100 * upstream / norm(upstream)

inflow <- as.data.frame(rbind(upstream_vec, pt0)) %>%
  setNames(c("x", "y")) %>% 
  mutate(x = x + center_coords[["x"]], y = y + center_coords[["y"]]) %>% 
  sf::st_as_sf(coords = c("x", "y"), crs = 31370)
...
- target area | `sample_support_code` 
  ...
  - [ ] streams
  - [ ] springs
- [ ] water everywhere... what if there is more water in the target area?
