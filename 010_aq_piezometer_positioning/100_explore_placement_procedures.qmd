...
stream_test_data <- data %>%
  filter(
    sample_support_code == "watercourse_segment",
...
get_stream_tangent <- function(
  ...
  x <- stream_curve$x
  y <- stream_curve$y
  stream_curve$dx <- predict(sm.spline(t, x), t, 1)
  stream_curve$dy <- predict(sm.spline(t, y), t, 1)

  # append second derivative
  if (second) {
    stream_curve$dx2 <- predict(sm.spline(t, x), t, 2)
    stream_curve$dy2 <- predict(sm.spline(t, y), t, 2)
  }

  tangent_norm <- function(x, y) sqrt((x**2) + (y**2))
  stream_curve$n <- tangent_norm(stream_curve$dx, stream_curve$dy)

  # optionally normalize 
  if (normed) {
    stream_curve$dx <- stream_curve$dx / stream_curve$n
    stream_curve$dy <- stream_curve$dy / stream_curve$n

    if (second) {
      n2 <- tangent_norm(stream_curve$dx2, stream_curve$dy2)
      stream_curve$dx2 <- stream_curve$dx2 / n2
      stream_curve$dy2 <- stream_curve$dy2 / n2
    }
  }
    
  if (append) return(stream_curve)
  if (second) return(stream_curve[, c("rank", "dx2", "dy2")])
  return(stream_curve[, c("rank", "dx", "dy")])
}
...
get_normal <- function(
  ...
  tangents <- lapply(1:length(dx), FUN = function(t) c(dx[t], dy[t]))  
  ...
  # head(cbind(stream_curve, normals))
  if (append) {
...
get_curvature_direction <- function(stream_curve, direction = TRUE, smooth = NA) {
  ...
  # v1 <- stream_curve[, c("dx", "dy")]
  # v2 <- stream_curve[, c("dx2", "dy2")]
  cx <- diff(stream_curve$x)
  cy <- diff(stream_curve$y)
  dx <- stream_curve$dx[1:length(cx)]
  dy <- stream_curve$dy[1:length(cy)]
  nx <- stream_normal$nx[1:length(cx)]
  ny <- stream_normal$ny[1:length(cy)]

  tangent_norm <- function(x, y) sqrt((x**2) + (y**2))
  l <- tangent_norm(cx, cy)
  cx <- cx / l
  cy <- cy / l
  ...
  #mat <- function(x, y) t(as.matrix(c(x, y)))
  curvatures <- c(sapply(
    1:length(l),
  ...
  if (direction) {
    dirn <- 2*(as.numeric(curvatures < 0)-0.5)
    dirn[is.na(dirn)] <- 0
    return(dirn)
  }
...
# tangent_norm <- function(sc) sqrt((sc$dx**2) + (sc$dy**2))
...
x <- test_curve$x
y <- test_curve$y
dx <- test_curve$dx * scale
dy <- test_curve$dy * scale
dx2 <- test_curve$dx2 * scale
dy2 <- test_curve$dy2 * scale
...
plot(x, y)
graphics::arrows(x, y, x+dx, y+dy, col = "darkgray", length = 0.1)
# graphics::arrows(x, y, x+nx, y+ny, col = "lightblue", length = 0.1)
# graphics::arrows(x-dx2, y-dy2, x+dx2, y+dy2, col = "lightblue", length = 0.1)
graphics::arrows(
  x[nz], y[nz],
...
stream_list <- data %>%
  ...
  select(unit_id)
stream_data <- lapply(stream_list, FUN = assemble_targeting)
```
