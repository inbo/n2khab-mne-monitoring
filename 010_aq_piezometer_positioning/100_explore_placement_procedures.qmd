...
all_inflows <- bind_rows(lapply(
    target_data, FUN = function(td) cbind(td$location_code, sf::st_as_sf(td$inflow))
  ))
...
# and map them out
map_types <- c("Esri.WorldImagery", "OpenStreetMap")

m1 <- mapview(all_polygons,
  map.types = map_types, col.regions = "lightblue")

m2 <- mapview(all_inflows, map.types = map_types, col.regions = "darkblue")
