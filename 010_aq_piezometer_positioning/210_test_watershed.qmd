...
dim(test_smoothed)[1:2] == dim(mask)
...
par(mfrow = c(2, 2))
plot(test_smoothed, col = gray.colors(256))
plot_points(test_ws_drained, col = "green")
plot(test_ws[["watershed"]])
plot_points(test_ws, col = "red")
image(mask)
plot(test_ws_drained[["watershed"]])
plot_points(test_ws_drained, col = "red")
...
- However, the masking is inefficient in R and computationally inefficient.
- This is different from *ex post* joining, because flows might just temporarily pass the water body (which, acknowledged, is unlikely).
