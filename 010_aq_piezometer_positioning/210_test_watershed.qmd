...
A solution for future exploration would be to add prior knowledge:
provide a mask for the water body of interest and join all inflow (i.e. once `coords_next` is in the `mask`, set it to centroid).
This could work for standing waters or stream segments.
