...
test_resmoothed <- smooth_raster(test_resampled, sigma = 2)
# test_watershed_raw <- compute_watershed(test_raster)[["watershed"]]
# test_watershed_smooth <- compute_watershed(test_smoothed)[["watershed"]]
test_watershed_resampled <- compute_watershed(test_resampled)[["watershed"]]
test_watershed_resmoothed <- compute_watershed(test_resmoothed)[["watershed"]]
# spat_raster <- test_resampled
# areas <- test_watershed[["watershed"]]
# sinks <- test_watershed[["sinks"]]
...
plot(0)#test_watershed_raw)
plot(0)#test_watershed_smoothed)
plot(test_watershed_resampled)
plot(test_watershed_resmoothed)


# TODO unclear whether the t()/flipping/mirroring should be done.
# TODO matrix must be square for Gaussian/Euclid ...
